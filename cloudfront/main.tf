...
locals {
  ...
  react_app            = "App.js"
  aws_ami_backend_name = "adudych-backend"
...
resource "aws_cloudfront_distribution" "website_cdn_redirect" {
  ...
  origin {
    origin_id           = "origin-bucket-redirect-${var.redirect_domain_name}"
  ...
  default_cache_behavior {
    ...
    default_ttl            = "300"
    max_ttl                = "1200"
    target_origin_id       = "origin-bucket-redirect-${var.redirect_domain_name}"
    viewer_protocol_policy = var.viewer_protocol_policy
    compress               = true
    forwarded_values {
      query_string = false
...
resource "null_resource" "cdn_invalidation" {
  ...
  triggers = {
    bucket_object_change = join("", [for k in aws_s3_bucket_object.website : k.etag])
  }
  depends_on = [
